...
# dd_director + the per-map LTX, never baked here. Ordered for the report/audit; `mutant` is one pooled bag.
CATEGORIES = [
    "mutant",
...
# install already plays it. Doubling is handled at RUNTIME by the base-veto (dd_director
# mutes the base's copy of a sound we ship, keyed off dd_blockdata). _get_active_channels is
...
MOD = HERE.parent                      # DiegeticDread repo root
GDATA = MOD / "gamedata"
...
# ----------------------------------------------------------------------------
# The shipped model: one-shot spook channels the standalone director (dd_director) plays. Every dark
...
def _build_veto_overlay(effects):
    ...
    out = ["; GENERATED by tools/build.py - do not edit. Static DLTX veto: removes DiegeticDread' own",
...
# placement is the sound's own source-channel spawn band through the vanilla formula (dd_director emit),
...
# by (mood, exact-settings-tuple): one deployed channel dd_eff_<mood>_<n> per distinct tuple,
# so a source channel's period/distance/indoor/height survive exactly (provenance-faithful).
# The mood is only a tag for the MCM knobs; dd_director reads it off the <mood> in the name.
...
def _report_dangling_dread(effects, root):
    """Report dd_spooks_metadata dread entries whose deployed name matches no shipped sound - a stale hash after
    an upstream re-encode silently no-ops (the sound reverts to play-everywhere). Read-only; the director owns
    applying the file. The sounds table is keyed by deployed name, one ["name"] = { dread = ... } per line."""
    path = Path(root) / "scripts" / "dd_spooks_metadata.script"
    if not path.exists():
        return
...
def cmd_deploy(a):
    ...
    # are no low/med/high subdirs. There is no engine sound_channels.ltx for our content either - the director
    # reads dd_sound_metadata, not channels.
    for cat in sorted(effects):
        for e in effects[cat]:
    ...
    # dd_sound_metadata: category -> its sounds, each a named-field table. The director reads THIS instead of
    # sound_channels.ltx, and also flattens it into xsound.load_meta for the delivered-loudness readout. Fields:
    #   path              deployed sound path (also the xsound metadata key; its 2nd segment is the category)
    #   ch_min, ch_max    the source channel's SPAWN band - the pair vanilla update_ambient transforms to place
    #                     the sound (random((max+min)/2, max)/2 outdoors), recovered by _build_source_band_map +
    #                     _resolve_band (same-author first), the file's own blob pair for folder-only sounds.
    #   mn, mx            the ATTENUATION range from the deployed ogg blob (the engine's fade curve).
    #   h                 the ORIGINAL source-channel elevation (_build_source_height_map, highest non-zero wins).
    #   indoor            the channel's flag for the vanilla volume rule.
    #   lufs, crest, bv   measured loudness, crest factor, and the blob base_volume - the sound's audio facts,
    #                     consumed by xsound.compute_delivered_loudness. Anomaly Lua cannot list a directory at
    #                     runtime, so the list ships as data. No play rules here (env/requires/dread live elsewhere).
    man = ["--- dd_sound_metadata: GENERATED by tools/build.py, do not edit. Category -> its flat sound list for",
           "--- the director (read instead of sound_channels.ltx), also fed to xsound.load_meta. Each row is a",
           "--- named-field table: path, ch_min/ch_max (source SPAWN band), mn/mx (ogg-blob ATTENUATION range),",
           "--- h (source-channel height), indoor (vanilla volume flag), lufs/crest/bv (measured audio facts).",
           "--- Dread is NOT here - it is per-sound business metadata in dd_spooks_metadata.",
           "-- @novalidate(reason:\"generated data table, regenerated by tools/build.py\")",
           "categories = {"]
    ...
    for cat in sorted(effects):
        ...
        for e in effects[cat]:
            ...
            bv = b[2] if b else 1.0                                        # blob base_volume (loudness gain)
            h = hmap.get(e["source_path"], 0)                 # aggregated source height (any pack), else 0
            bmn, bmx, ind = bands[id(e)]
            rows.append(
                '{ path="zs\\\\%s\\\\%s", ch_min=%s, ch_max=%s, mn=%s, mx=%s, h=%s, indoor=%s, lufs=%s, crest=%s, bv=%s }' % (
                    cat, n, round(bmn, 1), round(bmx, 1), mn, mx, h, "true" if ind else "false",
                    _num(lufs_map.get(n), 1), _num(e.get("crest", 0), 1), _num(bv, 3)))
        man.append('\t["%s"] = {' % cat)
        man += _pack(rows, "\t\t")
        man.append("\t},")
    man += ["}"]
    (root / "scripts").mkdir(parents=True, exist_ok=True)
    (root / "scripts" / "dd_sound_metadata.script").write_text("\n".join(man) + "\n", encoding="utf-8")
    ...
    (env / "mod_sound_channels_diegeticdread.ltx").write_text(dltx, encoding="utf-8")
...
def cmd_add(a):
    ...
    NOT in the tree - it is per-sound in dd_spooks_metadata (never wiped), so it is preserved regardless.
...
def cmd_verify(a):
    """Referential integrity of the committed config against the committed sound tree: every dd_sound_metadata
    ...
    cfg = root / "scripts" / "dd_sound_metadata.script"
    snd = root / "sounds"
    if not cfg.exists():
        print("  verify: dd_sound_metadata.script absent - run `deploy` first")
        return
...
def cmd_audit(a):
    """Reach audit over dd_sound_metadata, read-only, no ffmpeg. Per sound, the reach verdict at its spawn
    ...
    ch_max)/2, dd_director.script:509-527) and only pulls CLOSER by dread, so ch_max/2 (the base-observer far
    ...
    cfg = root / "scripts" / "dd_sound_metadata.script"
    if not cfg.exists():
        print("  audit: dd_sound_metadata.script absent - run `deploy` first")
        return
...
def cmd_rebuild(a):
    ...
    and re-emits the whole corpus FLAT into <cat>/ (dread lives per-sound in dd_spooks_metadata, not in the
