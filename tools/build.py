...
def _report_dangling_dread(effects, root):
    """Report as_spooks_metadata dread entries whose deployed name matches no shipped sound - a stale hash after
    ...
    path = Path(root) / "scripts" / "as_spooks_metadata.script"
    if not path.exists():
        return
...
def cmd_deploy(a):
    ...
    (root / "scripts" / "as_manifest.script").unlink(missing_ok=True)   # renamed to as_sound_metadata
    (root / "scripts" / "as_sound_config_gen.script").unlink(missing_ok=True)   # renamed to as_sound_metadata

    # Emit each sound FLAT into its category directory: zs\<category>\<name>.ogg. Dread is NOT a directory -
    # it lives per-sound in as_spooks_metadata (the business-metadata override the director reads), so there
    # are no low/med/high subdirs. There is no engine sound_channels.ltx for our content either - the director
    # reads as_sound_metadata, not channels.
    for cat in sorted(effects):
        for e in effects[cat]:
    ...
    # as_sound_metadata: category -> its sounds, each a named-field table. The director reads THIS instead of
    ...
    man = ["--- as_sound_metadata: GENERATED by tools/build.py, do not edit. Category -> its flat sound list for",
           ...
           "--- Dread is NOT here - it is per-sound business metadata in as_spooks_metadata.",
    ...
    (root / "scripts" / "as_sound_metadata.script").write_text("\n".join(man) + "\n", encoding="utf-8")
...
def cmd_add(a):
    ...
    NOT in the tree - it is per-sound in as_spooks_metadata (never wiped), so it is preserved regardless.
