...
def _veto_dltx(effects):
    ...
    When our removals empty a channel - every sound in it is ours in some config - also emit
    `>sounds = ambient\\no_sound`. A channel loaded as a System A bed CTDs on an empty `sounds`
    (Environment_misc.cpp:105-108), and whether a channel is a bed varies by install, so the guard keys
    off "we emptied it", not off bed detection. Only fully-emptied channels get the placeholder, so a
    partially-removed channel keeps its own sounds and is not diluted.
    ...
    guard = set()    # channels our removals fully empty in some config -> need a no_sound placeholder
            ...
            for chan, entries in _sc_channel_sounds(sc).items():
                ...
                for raw in entries:
                    ...
                    if f.is_file() and ah(f) in want:
                        ...
                        matched += 1
                if entries and matched == len(entries):
                    guard.add(chan)
    out = ["; GENERATED by tools/merge.py - do not edit. Static DLTX veto: removes AlifeSpooks' own",
           ...
           "; A channel our removals fully empty gets ambient\\no_sound so a bed never asserts on empty.",
           ""]
...
def cmd_deploy(a):
    ...
    # as_director. Anomaly Lua cannot list a directory at runtime, so the sound list ships as data. Paths
    # only - no play rules (env/requires/gates live in as_director, not the manifest).
